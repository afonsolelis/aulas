...
9. **Conteúdo de avaliação nunca entra no repositório.** O repositório é público (`github.com/afonsolelis/aulas`), de modo que enunciado de prova, alternativa correta e gabarito versionados ficam legíveis pela turma dias antes da aula. Enunciado de prova mora em `prova_provas`/`prova_partes` no Supabase — tabelas com RLS sem policy e `revoke all`, acessíveis apenas pela função `prova_abrir(slug, token)`, que confere o hash SHA-256 do token do professor (`supabase/prova-schema.sql`). O SQL que popula essas tabelas fica em `supabase/private/`, ignorado pelo git; o formato está em `supabase/prova-seed.example.sql`. O deck busca o texto no momento da projeção e não contém uma linha do enunciado.
   > **Pendência conhecida:** os seeds de quiz e de TBL (`supabase/quiz-seed-*.sql`, `supabase/tbl-seed-*.sql`) ainda trazem enunciado, alternativa correta e explicação em texto claro no repositório público.
