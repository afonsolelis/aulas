...
--
-- O seed grava 90 segundos em cada questão. O campo "Tempo" do painel
-- (lesson-N-host.html) chama quiz_tempo e troca esse valor para todas as
-- questões da sala. Tudo que conta o tempo já lê quiz_questions.segundos —
-- o fechamento automático em quiz_fechar_expirada, o prazo e a pontuação
-- por rapidez em quiz_responder, o cronômetro do aparelho em quiz_estado e
-- o do painel em quiz_host —, então nenhuma outra função nem a página do
-- estudante precisa mudar.
--
--   quiz_tempo(slug, token)            lê o tempo atual da sala
--   quiz_tempo(slug, token, segundos)  aplica de 10 a 600 segundos
--
-- A troca é recusada com a pergunta aberta: o prazo de fechamento, o
-- cronômetro já projetado e a pontuação de quem respondeu passariam a
-- seguir relógios diferentes. Entre perguntas (lobby, revelação ou
-- encerrada), vale a partir da próxima que abrir. O valor fica na sala e
-- atravessa o Reiniciar; reaplicar o seed volta a 90.
--
-- Aplicar depois de quiz-funcoes.sql. Cria só a função quiz_tempo; não
-- altera tabela nem outra função, e pode ser aplicado com sala em uso.
-- =====================================================================
...
begin
  ...
  if p_segundos is not null then
    ...
    -- Uma pergunta vencida ainda consta como aberta até a próxima consulta;
    -- fechá-la antes evita recusar a troca por um estado já expirado.
