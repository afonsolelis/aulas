...
-- minutos: noventa segundos de resposta por questão e o tempo restante
...
--
-- A pergunta fecha sozinha ao fim do tempo, sem depender de comando do
-- professor, e o resultado é exibido de imediato. A décima questão tem
-- peso 2: a última rodada mantém a disputa aberta até o encerramento.
--
...
with novas as (
  ...
  ('stakeholders-m7-a6', 1,
   ...
   '["Fora do registro, porque o efeito que a área teme não se confirma na parametrização definida",
     ...
     "Fora do registro de partes e dentro do registro de riscos, por se tratar de suposição infundada"]'::jsonb, 90, 1, 'Definição de parte interessada', 'seção 2'),
  ...
  ('stakeholders-m7-a6', 2,
   ...
   '["Solução em operação sem quem detenha condição técnica de mantê-la e de operar o que foi entregue",
     ...
     "Perda de prioridade do projeto na disputa da organização por recursos e por decisão de investimento"]'::jsonb, 90, 1, 'Categorias na implantação', 'seção 4'),
  ...
  ('stakeholders-m7-a6', 3,
   ...
   '["Manter informado; comunicação regular do andamento, com contribuição em requisito e teste",
     ...
     "Monitorar; acompanhamento periódico, sem ação dedicada de engajamento pela equipe"]'::jsonb, 90, 1, 'Poder e interesse', 'seção 5'),
  ...
  ('stakeholders-m7-a6', 4,
   ...
   '["Permanece dominante, porque o poder e a legitimidade que já detinha não se alteraram no período",
     ...
     "Torna-se definitiva, porque a urgência se acrescenta ao poder e à legitimidade que já detinha"]'::jsonb, 90, 1, 'Saliência', 'seção 6'),
  ...
  ('stakeholders-m7-a6', 5,
   ...
   '["De resistente a apoiador; endereçar o efeito que a mudança produz sobre o interesse da área",
     ...
     "De desinformado a neutro; apresentar a razão de negócio que justificou a implantação"]'::jsonb, 90, 1, 'Engajamento atual e desejado', 'seção 7'),
  ...
  ('stakeholders-m7-a6', 6,
   ...
   '["Escalonamento ao comitê diretivo, para decidir entre a exigência fiscal e a data acordada",
     ...
     "Antecipação de controle, com a área fiscal participando da definição do requisito"]'::jsonb, 90, 1, 'Estratégias de engajamento', 'seção 8'),
  ...
  ('stakeholders-m7-a6', 7,
   ...
   '["A periodicidade adequada: o comunicado por marco deveria ter sido convertido em envio semanal",
     ...
     "O destinatário correto: o comunicado deveria ter sido dirigido ao dono do processo da área"]'::jsonb, 90, 1, 'Plano de comunicação', 'seção 9'),
  ...
  ('stakeholders-m7-a6', 8,
   ...
   '["Registro do escopo da delegação, com a validação final reservada ao titular do papel",
     ...
     "Substituição das atas de homologação por relatório técnico emitido pela equipe de projeto"]'::jsonb, 90, 1, 'Delegação e legitimidade', 'seção 10'),
  ...
  ('stakeholders-m7-a6', 9,
   ...
   '["Ausência de consultado na atividade, que priva a decisão do parecer da área de controle",
     ...
     "Violação da regra de aprovador único, que deixa a decisão final sem responsável definido"]'::jsonb, 90, 1, 'Matriz de responsabilidades', 'seção 11'),
  ...
  ('stakeholders-m7-a6', 10,
   ...
   '["Aceito, porque o atendimento simultâneo das duas exigências demonstra que a negociação foi bem conduzida",
     ...
     "Devolvido, porque a decisão sobre a etapa de aprovação deveria ter sido escalada ao comitê diretivo antes do registro"]'::jsonb, 90, 2, 'Interesses conflitantes', 'seção 12')
  returning id, ordem
)
