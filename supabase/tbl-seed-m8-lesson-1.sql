...
values (
  ...
  'Uma instituição de pagamento com 4,2 milhões de contas oferece transferência PIX por comando em linguagem natural, no aplicativo e no canal de mensagens. O canal responde por 23% das transações e por 31% do volume iniciado entre 20h e 6h. No último trimestre foram registradas 1.870 ocorrências de transação contestada, com perda de R$ 3,1 milhões. A classificação vigente não distingue a origem do incidente: enunciado reconhecido de forma incorreta, chave de destino divergente do beneficiário pretendido e ordem emitida pelo próprio titular são registrados sob o mesmo código de contestação. O conselho determinou a redução de 60% da perda em dois trimestres, sem queda na conversão do canal. O Banco Central exige o registro da transação, mantém o Mecanismo Especial de Devolução para os casos contestados e limita o valor das transações noturnas. A equipe dispõe de um trimestre para implantar as respostas, e as decisões de arquitetura serão tomadas nesta semana.',
  '[
    ...
    {"rotulo":"Janela de implantação","valor":"um trimestre","nota":"respostas em produção"},
