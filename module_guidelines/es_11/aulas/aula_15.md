# Aula 15 - OTIMIZAÇÃO DE DATA WAREHOUSES

## Descrição da Aula
Esta instrução tem como objetivo capacitar os alunos a aplicar técnicas de otimização em Data Warehouses, com foco em melhorar o desempenho de consultas analíticas (OLAP) em grandes volumes de dados. Serão abordadas estratégias como indexação e particionamento de tabelas fato, uso de views materializadas e agregações pré-processadas, bem como clustering e ordenação. A instrução destacará práticas específicas de tuning para os mecanismos ClickHouse e PostgreSQL, e incluirá exercícios de benchmarking para análise de desempenho em cenários reais.
