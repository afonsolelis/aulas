...
/**
 * TBL da Aula 15 do Módulo 11 — sequência de decisões de observabilidade.
 *
...
const ALUNO = 'file://' + path.resolve(__dirname, '../pages/module-11-eng-software/tbl/lesson-15-tbl.html');
const PAINEL = 'file://' + path.resolve(__dirname, '../pages/module-11-eng-software/tbl/lesson-15-painel.html');
...
const QUESTOES = [
  { ordem: 1, categoria: 'O que medir', titulo: 'O indicador que teria falado nas 41 noites',
    pergunta: 'Qual dimensão a equipe instrumenta primeiro?', dado_novo: null },
  { ordem: 2, categoria: 'Registro de execução e linhagem', titulo: 'A pergunta da auditoria: quais dias, quais lojas, qual volume',
    pergunta: 'O que a equipe constrói para que a pergunta seja respondível?', dado_novo: null },
  { ordem: 3, categoria: 'Detecção da anomalia', titulo: 'Contra o que comparar o número desta noite',
    pergunta: 'Qual referência a equipe adota para decidir que a carga está anômala?',
    dado_novo: {
      titulo: 'A distribuição da perda entre as lojas',
      texto: 'No agregado nacional a perda é de 1,2% das linhas, dentro da variação semanal.',
      evidencias: ['Nas 34 lojas afetadas a perda alcança 19% em noites específicas.'],
      pergunta: 'A sua escolha sobrevive a um sinal que não existe no agregado?'
    } }
...
function salaFalsa() {
  ...
  const estado = (id: string | null) => {
    ...
    const base: Record<string, unknown> = {
      ...
      caso_titulo: 'As quarenta e uma noites em que a carga terminou bem',
      caso_texto: 'Rede de 380 lojas cujo armazém analítico perde dado sem acusar erro.',
      contexto: [{ rotulo: 'Execuções sem erro', valor: '41 noites', nota: 'zero linhas rejeitadas' }],
...
test.describe('TBL · Aula 15 do Módulo 11 · observabilidade de pipelines', () => {
  test('sequência de questões, com discussão e segunda decisão em cada uma', async ({ browser }) => {
    ...
    // parado.
    await expect(professor.locator('#url-aluno')).toContainText('lesson-15-tbl.html');
    ...
    await expect(ana.locator('#conteudo')).toContainText('As quarenta e uma noites em que a carga terminou bem');
    await expect(ana.locator('button.opcao')).toHaveCount(0);
    await expect(ana.locator('#conteudo')).not.toContainText('O indicador que teria falado nas 41 noites');
    ...
    await expect(ana.locator('#conteudo')).toContainText('O que medir');
    ...
    await votar(ana, 0, 'A completude por partição enxerga a perda onde ela acontece.');
    await votar(bruno, 3, 'A reconciliação verifica o fato em vez de inferir do hábito.');
    await expect(professor.locator('#decidiram')).toHaveText('2');

    // A decisão pode ser alterada enquanto a rodada está aberta.
    await votar(ana, 1, 'A atualidade entra em operação já na primeira noite.');
    ...
    await votar(ana, 1, 'Mantenho a atualidade depois de ouvir o custo das demais.');
    await votar(bruno, 1, 'A discussão me deslocou da reconciliação para a atualidade.');
    ...
    await expect(ana.locator('#conteudo')).toContainText('Registro de execução e linhagem');
    await expect(ana.locator('button.opcao[aria-pressed="true"]')).toHaveCount(0);
    await expect(ana.locator('.barras')).toHaveCount(0);
    await votar(ana, 0, 'O registro de execução torna a carga consultável por SQL.');
    await votar(bruno, 2, 'A camada bruta permite recomputar qualquer dia.');
    await professor.click('#btn-avancar');   // discussão
    await professor.click('#btn-avancar');   // segunda decisão
    await votar(ana, 0, 'Sigo com o registro de execução por loja.');
    await votar(bruno, 0, 'Aceito o registro de execução após a objeção sobre a linhagem.');
    await professor.click('#btn-avancar');   // síntese
    ...
    await expect(ana.locator('#conteudo')).not.toContainText('distribuição da perda');
    await votar(ana, 0, 'A referência móvel absorve a sazonalidade semanal.');
    await votar(bruno, 3, 'A previsão por modelo acomoda feriado e promoção.');

    await professor.click('#btn-avancar');
    await sincronizar(ana);
    await expect(ana.locator('#selo-fase')).toContainText('Discussão · questão 3 de 3');
    await expect(ana.locator('#conteudo')).toContainText('A distribuição da perda entre as lojas');

    await professor.click('#btn-avancar');
    await sincronizar(ana);
    await expect(ana.locator('#conteudo')).toContainText('sobrevive a um sinal que não existe no agregado');
    await votar(ana, 1, 'Passo ao invariante: a referência móvel se contamina pela própria perda.');
    await votar(bruno, 3, 'Mantenho a previsão, que trata a loja de comportamento irregular.');
    ...
    await expect(ana.locator('#conteudo')).toContainText('Cinco decisões de instrumentação');
    await expect(ana.locator('#conteudo')).toContainText('O que medir');
    await expect(ana.locator('#conteudo')).toContainText('Detecção da anomalia');
